...
class GF_Marketing_360_Payments
{
	...
	public static function rest_list_m360_accounts(WP_REST_Request $request)
	{
		...
		$username = sanitize_email( $request['username'] );
		$password = sanitize_text_field( $request['password'] );

		$token = self::username_password_get_access_token($username, $password);

		if (is_wp_error($token)) {
			return new WP_REST_Response($token->get_error_message(), 401);
		} else {
			$accounts = self::get_m360_accounts($token);

			if (is_wp_error($accounts)) {
				return new WP_REST_Response($accounts->get_error_message(), 400);
			}
	...
	public static function get_customer_id($email)
	{
		...
		if (!$customers) {
			gf_m360()->log_debug(__METHOD__ . '(): No existing customer found, creating new customer for: ' . $email);
			$customer = self::create_customer($email);
			if (is_wp_error($customer)) {
				gf_m360()->log_error(__METHOD__ . '(): Customer creation failed for email: ' . $email . '. ' . $customer->get_error_message());
				return $customer;
			}
			gf_m360()->log_debug(__METHOD__ . '(): New customer created with ID: ' . $customer['id']);
			return $customer['id'];
		} else {
			gf_m360()->log_debug(__METHOD__ . '(): Existing customer found with ID: ' . $customers[0]['id']);
