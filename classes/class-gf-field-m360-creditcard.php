...
class GF_Field_M360_CreditCard extends GF_Field
{
	...
	public function get_field_input($form, $value = '', $entry = null)
	{
		...
		if ($is_admin) {
			...
			$stripe_account_id = is_object($account_details) ? $account_details->stripeAccountId : (is_array($account_details) ? $account_details['stripeAccountId'] ?? '' : '');
			...
			ob_start(); ?>
				...
				<script>
					...
					const stripeAccountId = "<?php esc_html_e($stripe_account_id); ?>";
					...
					document.addEventListener('gform_payment_requires_action', function (event) {
						const clientSecret = event.detail.client_secret;
						stripe.handleCardAction(clientSecret).then(function (result) {
							if (result.error) {
								jQuery('#<?php esc_html_e($input_id); ?>').val(JSON.stringify({ error: result.error }));
							} else {
								jQuery('#<?php esc_html_e($input_id); ?>').val(JSON.stringify({ payment_intent: result.paymentIntent }));
							}
							jQuery('#gform_submit_button_<?php esc_html_e($form_id); ?>').trigger('click');
						});
					});
				</script>
			...
			// Frontend but no Stripe key — account not fully connected.
			ob_start(); ?>
			<div class="gfield_description validation_message">
				<?php esc_html_e( 'Payment field could not be loaded. Please contact the site administrator.', 'gravityformsm360' ); ?>
			</div>
			<?php return ob_get_clean();
		}
	}
